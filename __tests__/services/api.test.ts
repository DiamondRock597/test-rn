...
describe('API', () => {
  ...
  it('should create axios instance with the expected base URL and headers', () => {
    expect(axiosInstance.defaults.baseURL).toBe('https://artjoms-spole.fly.dev');
  ...
  it('should call axiosInstance.post with correct arguments', async () => {
    ...
    const result = await API.post('/test', { foo: 'bar' }, { Authorization: 'token' });
  ...
  it('should call axiosInstance.put with correct arguments', async () => {
    ...
    const result = await API.put('/test', { value: 42 }, { Authorization: 'token' });
